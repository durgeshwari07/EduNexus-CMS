...
import { 
  ...
  ChevronLeft, ChevronRight, Activity, Bell, Settings, LogOut 
} from 'lucide-react';
    ...
    <div className="animate-in fade-in duration-500 max-w-full">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium">Welcome back, Admin. Here is what's happening today.</p>
        </div>
        <div className="flex gap-3">
          ...
          <button className="flex items-center gap-2 bg-[#136dec] text-white px-5 py-2.5 rounded-xl text-xs font-black shadow-lg shadow-[#136dec]/20 hover:bg-blue-700 transition-all active:scale-95">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white rounded-[24px] border border-slate-200 overflow-hidden shadow-sm">
            <div className="p-6 border-b flex justify-between items-center">
              <h4 className="font-black text-slate-900 text-sm uppercase tracking-widest">Pending Teacher Requests</h4>
              <button className="text-xs font-bold text-[#136dec] hover:underline">View All</button>
            </div>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {pendingTeachers.length > 0 ? (
                    pendingTeachers.map((t) => (
                      <tr key={t.id} className="hover:bg-slate-50 transition-colors group">
                        <td className="px-6 py-4 flex items-center gap-3">
                          <div className="size-9 bg-slate-100 rounded-full flex items-center justify-center text-[11px] font-black text-slate-600 border border-slate-200">
                            {t.initial || 'SJ'}
                          </div>
                          <span className="font-bold text-slate-700">{t.name}</span>
                        </td>
                        <td className="px-6 py-4 text-slate-500 font-medium">{t.dept}</td>
                        <td className="px-6 py-4 text-slate-400 text-xs">Oct 12, 2023</td>
                        <td className="px-6 py-4 text-right">
                          {isAdmin && (
                            <div className="flex justify-end gap-3">
                              <button onClick={() => onApprove(t.id)} className="bg-[#136dec] text-white px-4 py-1.5 rounded-lg text-[10px] font-black hover:bg-blue-700 shadow-md shadow-blue-100 transition">Approve</button>
                              <button className="text-slate-400 font-bold text-[10px] hover:text-red-500 transition">Reject</button>
                            </div>
                          )}
                        </td>
                      </tr>
                    ...
                    <tr><td colSpan="4" className="px-6 py-10 text-center text-slate-400 italic font-medium">No pending requests at the moment.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          <div className="bg-white p-8 rounded-[24px] border border-slate-200 shadow-sm">
            <div className="flex justify-between items-center mb-8">
              <h4 className="font-black text-sm uppercase tracking-widest">Top Performing Departments</h4>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white rounded-[24px] border border-slate-200 p-7 shadow-sm">
            ...
            <div className="flex justify-between mb-8 items-center bg-slate-50 p-3 rounded-xl">
              <ChevronLeft size={16} className="cursor-pointer text-slate-400 hover:text-[#136dec]" />
              <span className="text-xs font-black text-slate-900">October 2023</span>
            ...
            <button className="w-full mt-8 py-3 text-[10px] font-black uppercase tracking-widest text-slate-400 border border-slate-100 rounded-xl hover:bg-slate-50 hover:text-[#136dec] transition-all">Full Schedule</button>
          </div>
          ...
          <div className="bg-gradient-to-br from-[#136dec] to-blue-700 p-8 rounded-[24px] text-white shadow-2xl shadow-blue-200 relative overflow-hidden group">
              ...
              <div className="flex justify-between items-end mb-4">
                 ...
                 <span className="text-[10px] font-bold opacity-60">390 GB / 500 GB</span>
              </div>
              <div className="w-full bg-white/20 h-2.5 rounded-full mb-8 overflow-hidden">
                 <div className="bg-white h-full rounded-full transition-all duration-1000" style={{width: '78%'}}></div>
              </div>
              <button className="w-full py-3 bg-white text-[#136dec] rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-50 transition-all shadow-lg">Upgrade Storage</button>
          </div>
        </div>
      </div>
    </div>
...
const ProgressBar = ({ label, val, width }) => (
  <div>
    <div className="flex justify-between text-[11px] font-black uppercase tracking-tight mb-3">
      <span className="text-slate-500">{label}</span><span className="text-[#136dec]">{val}</span>
    </div>
...
const CalendarItem = ({ day, month, title, sub, active }) => (
  <div className={`flex gap-5 p-4 rounded-2xl border transition-all ${active ? 'bg-[#136dec]/5 border-[#136dec]/10' : 'border-transparent hover:bg-slate-50'}`}>
    ...
    <div className="flex flex-col justify-center">
      <p className="text-sm font-black text-slate-900 leading-tight mb-0.5">{title}</p>
