...
// --- 4. MASTER DATA SYNC (UPDATED FOR HOD AUTO-FILL) ---
app.get('/api/dashboard/data', async (req, res) => {
    try {
        // Fetch departments including the HOD name for frontend auto-fill
...
app.post('/api/register-teacher', async (req, res) => {
    ...
    try {
        ...
        await db.query(sql, [d.first_name, d.middle_name || null, d.last_name, d.email.toLowerCase(), d.phone, d.dept_id, d.qualification, d.experience || 0, d.employmentType, d.username, hash]);
        res.json({ success: true });
    } catch (err) { res.status(500).json({ success: false, message: "Teacher Registration failed" }); }
});

// --- 6. CORE MANAGEMENT (UPDATED BATCH LOGIC) ---
app.post('/api/departments', async (req, res) => {
    try {
        await db.query('INSERT INTO departments (name, code, hod) VALUES (?, ?, ?)', [req.body.name, req.body.code, req.body.hod]);
        res.json({ success: true });
...
app.post('/api/batches', async (req, res) => {
    ...
    try {
        await db.query(
            'INSERT INTO batches (dept, batch, hod, year) VALUES (?, ?, ?, ?)', 
            [dept, batch, hod, year]
        );
        res.json({ success: true });
    } catch (err) { 
    ...
    try {
        const sql = `
            INSERT INTO students (
                name, enrollmentNo, prNo, email, phone, 
                batchId, semester, division, academicYear, 
                dob, address, guardianName, guardianPhone, 
                passCount, failCount, username, password, status
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Active')
        `;
...
app.put('/api/students/:id', async (req, res) => {
    ...
    try {
        const sql = `
            UPDATE students SET 
                name=?, prNo=?, email=?, phone=?, 
                semester=?, division=?, passCount=?, failCount=? 
            WHERE id = ?
        `;
        await db.query(sql, [
            s.name, s.prNo, s.email, s.phone, 
            s.semester, s.division, s.passCount, s.failCount, id
        ]);
        res.json({ success: true });
    } catch (err) {
