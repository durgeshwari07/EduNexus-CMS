...
// Global helper to prevent "sum is not defined" errors during rendering
const sum = (arr) => (arr || []).reduce((a, b) => a + Number(b), 0);
...
const FacultyPortal = () => {
  ...
  const handleLogin = async (e) => {
    ...
    setLoading(true);
    try {
      // Step 1: Fetch subjects assigned specifically to the logged-in teacher
      const res = await axios.get(`http://localhost:5000/api/faculty/data/${val}`);
      const subjects = res.data.subjects || res.data || [];
      
      // Step 2: Normalize student data structure for table rendering
      const normalizedSubjects = subjects.map(sub => ({
        ...sub,
        students: (sub.students || []).map(st => ({
          ...st,
  ...
  const saveModalData = async () => {
    ...
    try {
        // Step 3: Send totals to MySQL backend via save-marks route
        await axios.post('http://localhost:5000/api/faculty/save-marks', {
            studentId: modalStudentData.id,
            subjectId: sub.id,
            semester: sub.semester,
            marks: {
                ...
                isa2: sum(modalStudentData.isa?.isa2),
                isa3: sum(modalStudentData.isa?.isa3),
                theory: sum(modalStudentData.semMarks),
                practical: modalStudentData.practicalMarks
            }
        });
  ...
  const getStudentCalculations = (student) => {
    ...
    const i3 = sum(student.isa?.isa3);
    const sem = sum(student.semMarks);
    const prac = student.practicalMarks || 0;

    // Logic to consider the best 2 out of 3 ISAs for the total
    const isas = [i1, i2, i3].sort((a, b) => b - a);
    const topTwoTotal = isas[0] + (isas[1] || 0);
    
    return {
      isa1: i1, isa2: i2, isa3: i3,
      topTwo: topTwoTotal,
      grandTotal: topTwoTotal + sem + prac
    };
  };
    ...
    return (
      <div className="portal-theme login-wrapper">
        <div className="login-card">
          ...
          <form onSubmit={handleLogin}>
            <div className="form-group">
              <input name="user" type="text" className="form-input" placeholder="Teacher Username" style={{ textAlign: 'center' }} />
              {loginError && <div className="error-msg">⚠ Please enter valid username</div>}
            </div>
            <button type="submit" className="btn btn-primary" style={{ width: '100%' }}>
    ...
    <div className="portal-theme" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
      <div className="container" style={{ width: '100%', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div className="section-header" style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div className="logo" style={{ fontSize: '24px' }}>Uni<span>Desk</span></div>
            {view === 'dashboard' && <div style={{ color: 'var(--text-muted)', marginTop: '4px' }}>Teacher: <strong style={{ color: 'var(--text-main)' }}>{currentUser}</strong></div>}
          </div>
          <div style={{ display: 'flex', gap: '10px' }}>
            {view === 'grading' && <button className="btn btn-ghost" onClick={() => setView('dashboard')}>← Back</button>}
            <button className="btn btn-ghost" onClick={logout}>Sign Out</button>
        ...
        {view === 'dashboard' && (
          <div className="faculty-grid" style={{ width: '100%', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '24px' }}>
            {(db[currentUser] || []).map((sub, idx) => (
              <div className="card faculty-card" key={idx} onClick={() => openSubject(idx)} style={{ cursor: 'pointer' }}>
                <div style={{ width: '48px', height: '48px', background: 'linear-gradient(135deg, #4f46e5, #8b5cf6)', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 800, fontSize: '1.2rem', marginBottom: '16px' }}>
                ...
                <p style={{ color: 'var(--text-muted)' }}>{(sub.students || []).length} Students Assigned</p>
                <div style={{ marginTop: '24px', borderTop: '1px solid var(--border)', paddingTop: '12px', fontSize: '0.8rem' }}>
                  Semester: {sub.semester} | Batch: {sub.batchId}
                </div>
              </div>
            ))}
          </div>
        ...
        {view === 'grading' && currentSubject && (
          <div style={{ width: '100%' }}>
            <h2 style={{ fontSize: '1.75rem', fontWeight: 800 }}>{currentSubject.name} - Marks Entry</h2>
            <div className="table-wrapper">
              <table style={{ width: '100%' }}>
                <thead>
                  <tr style={{ background: '#f8fafc' }}>
                    <th style={{ textAlign: 'left', padding: '15px' }}>Student Name</th>
                    <th>ISA 1</th><th>ISA 2</th><th>ISA 3</th>
                    <th style={{ color: '#4f46e5' }}>Top 2</th>
                    <th>Theory</th>
                ...
                <tbody>
                  {(currentSubject.students || []).map((st, idx) => {
                     ...
                     return (
                      <tr key={st.id || idx}>
                        <td style={{ padding: '12px' }}>{st.name}</td>
                        <td className="clickable" onClick={() => openGradingModal(idx, 'ISA', 'isa1')}>{calc.isa1}</td>
                        <td className="clickable" onClick={() => openGradingModal(idx, 'ISA', 'isa2')}>{calc.isa2}</td>
                        <td className="clickable" onClick={() => openGradingModal(idx, 'ISA', 'isa3')}>{calc.isa3}</td>
                        <td style={{ fontWeight: 'bold', color: '#4f46e5' }}>{calc.topTwo}</td>
                        <td className="clickable" onClick={() => openGradingModal(idx, 'SEM')}>{sum(st.semMarks)}</td>
                        <td style={{ background: '#f8fafc' }}><strong>{calc.grandTotal}</strong></td>
                      </tr>
                     );
                  })}
                </tbody>
      ...
      {gradingModalOpen && modalStudentData && (
        <div className="modal-overlay">
          <div className="modal">
            <h3 style={{ marginBottom: '20px' }}>{gradingState.type} Entry - {gradingState.subType?.toUpperCase() || 'SEM'}</h3>
            <div className="modal-inputs">
              {(gradingState.type === 'PRACTICAL' ? [0] : (gradingState.type === 'ISA' ? modalStudentData.isa[gradingState.subType] : modalStudentData.semMarks)).map((val, qIdx) => (
                <div className="question-row" key={qIdx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
                  <label>Question {qIdx + 1}</label>
                  <input type="number" className="form-input" value={val} onChange={(e) => handleModalQuestionChange(qIdx, e.target.value)} style={{ width: '80px' }} />
                </div>
              ))}
            </div>
            {modalError && <p className="error-msg">{modalError}</p>}
            <button className="btn btn-primary" style={{ width: '100%', marginTop: '20px' }} onClick={saveModalData}>Save to Database</button>
            <button className="btn btn-ghost" style={{ width: '100%', marginTop: '10px' }} onClick={() => setGradingModalOpen(false)}>Cancel</button>
          </div>
        </div>
      )}
    </div>
