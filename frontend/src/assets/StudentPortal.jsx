  ...
  Object.values(marksDB).forEach((subjects) => {
    subjects.forEach((sub, idx) => {
      ...
      if (student) {
        ...
        // --- UPDATED: Use the dynamic credits from the subject ---
        const subjectCredits = sub.credits ? Number(sub.credits) : 4;
        
        tableRows.push({
          code: `SUB-0${idx + 1}`,
          name: sub.name,
          credits: subjectCredits, // Added to row data
          theoryMax: sub.theoryMax || sub.semMax || '--',
          theoryScore,
          practMax: sub.practicalMax || '--',
          practScore,
        ...
        // --- UPDATED: Calculate SGPA based on specific subject credits ---
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th>Theory (Max/Scored)</th>
                  <th>Pract (Max/Scored)</th>
                  <th>Total</th>
                  <th>Grade</th>
                </tr>
              </thead>
              <tbody>
                {tableRows.length === 0 ? (
                  <tr><td colSpan="7" style={{ textAlign: 'center', color: 'var(--text-muted)' }}>No records found for this student.</td></tr>
                ) : (
                  tableRows.map((row, i) => (
                    <tr key={i}>
                      <td style={{ fontWeight: 600, color: 'var(--text-secondary)' }}>{row.code}</td>
                      <td style={{ fontWeight: 500 }}>{row.name}</td>
                      {/* --- UPDATED: Display Credits --- */}
                      <td style={{ textAlign: 'center', fontWeight: 600 }}>{row.credits}</td>
                      <td dangerouslySetInnerHTML={{ __html: `${row.theoryMax} / <b>${row.theoryScore}</b>` }}></td>
                      <td>
                        {row.hasPractical 
                          ? <span dangerouslySetInnerHTML={{ __html: `${row.practMax} / <b>${row.practScore}</b>` }} /> 
