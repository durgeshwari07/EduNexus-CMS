    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100 p-4">
      <div className="flex flex-col md:flex-row bg-white rounded-[32px] shadow-2xl overflow-hidden w-full max-w-6xl md:h-[90vh]">
        <div className="hidden md:flex flex-1 flex-col justify-between bg-gradient-to-br from-purple-700 to-purple-500 text-white p-12">
          ...
          <div>
            <h1 className="text-4xl font-black mb-6">Join Our <br /> Faculty</h1>
            <p className="opacity-80">Manage your classes and subjects digitally.</p>
          </div>
          <div className="border-t border-white/20 pt-4 text-xs font-bold uppercase tracking-widest opacity-60">Pending Admin Approval</div>
        </div>

        <div className="flex-1 p-12 overflow-y-auto">
          <h2 className="text-3xl font-black text-slate-900">Teacher Registration</h2>
          <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4 mt-8">
            <input name="teacherName" placeholder="Full Name" className="p-3 bg-slate-50 border rounded-xl" required />
            <input name="email" type="email" placeholder="Email Address" className="p-3 bg-slate-50 border rounded-xl" required />
            <input name="phone" placeholder="Phone Number" className="p-3 bg-slate-50 border rounded-xl" />
            <select name="dept" className="p-3 bg-slate-50 border rounded-xl" required>
              <option value="Computer Science">Computer Science</option>
              <option value="Mathematics">Mathematics</option>
            </select>
            <input name="designation" placeholder="Designation" className="p-3 bg-slate-50 border rounded-xl" />
            <input name="employeeId" placeholder="Employee ID" className="p-3 bg-slate-50 border rounded-xl" />
            <input name="classes" placeholder="Classes (e.g. Sem 3, Sem 5)" className="col-span-2 p-3 bg-slate-50 border rounded-xl" />
            <input name="subjects" placeholder="Subjects" className="col-span-2 p-3 bg-slate-50 border rounded-xl" />
            <input name="username" placeholder="Username" className="col-span-2 p-3 bg-slate-50 border rounded-xl" required />
            ...
            <button type="submit" className="col-span-2 p-4 bg-purple-700 text-white font-black rounded-xl hover:bg-purple-950 transition-all">SUBMIT ACCESS REQUEST</button>
          </form>
        </div>
      </div>
    </div>
