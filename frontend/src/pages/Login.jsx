...
export default function LoginOtpApp({ onLoginSuccess }) {
  ...
  const [otp, setOtp] = useState(["", "", "", ""]);
  const [loading, setLoading] = useState(false);
  const [targetEmail, setTargetEmail] = useState(""); // Stores obfuscated email from backend
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    try {
      // Aligned with Backend: sends 'username'
      const response = await axios.post("http://localhost:5000/api/auth/request-otp", {
      ...
      if (response.data.success) {
        setTargetEmail(response.data.email); // The backend returns the email where OTP was sent
        alert(`OTP has been sent to ${response.data.email}`);
        setStep("otp");
      }
    } catch (err) {
      alert(err.response?.data?.message || "Invalid credentials for selected role");
  ...
  const verifyOtp = async () => {
    ...
    setLoading(true);
    try {
      // Sends the email (returned by request-otp) and the code
      const response = await axios.post("http://localhost:5000/api/auth/verify-otp", {
        email: targetEmail, 
        otp: otpString
      });
      ...
      if (response.data.success) {
        // IMPORTANT: response.data.user now contains the JWT token from our updated Backend
        onLoginSuccess(response.data.role, response.data.user);
      }
    } catch (err) {
        ...
        <div className="auth-card">
          ...
          <div className="form-panel">
            ...
            >
              ...
              <div className="slide">
                <h2 className="form-title">Check Your Email</h2>
                <p className="form-subtitle">OTP sent to {targetEmail || 'your email'}</p>
