...
import React, { useState, useMemo } from 'react';
import { Plus, X, LayoutGrid, Search, Building2, User, Loader2 } from 'lucide-react';
import DepartmentCard from '../components/DepartmentCard';
...
export default function Departments({ userRole, departments = [], onAddDept, onDeleteDept }) {
  ...
  // Uses optional chaining to prevent crashes if a database row is malformed
  const filteredDepts = useMemo(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Keys here MUST match your MySQL columns: name, code, hod
      await onAddDept({
        name: formData.get('deptName'),
        code: formData.get('deptCode'),
        hod: formData.get('hod'),
      ...
      alert("Failed to save to database. Check server connection.");
    } finally {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-7xl mx-auto p-6 space-y-8 font-sans">
      ...
      <div className="space-y-1">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-4xl font-black text-slate-900 tracking-tight leading-tight">Institutional Departments</h1>
            <p className="text-slate-500 font-medium mt-1">Manage academic branches, HOD assignments, and curriculum codes.</p>
          </div>
          {isAdmin && (
            <button 
              onClick={() => setIsDrawerOpen(true)} 
              className="flex items-center gap-2 bg-[#136dec] text-white px-6 py-3 rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-lg shadow-blue-500/20 hover:bg-blue-700 transition-all active:scale-95"
            >
              <Plus size={16} /> Add New Department
            </button>
          )}
      ...
      {filteredDepts.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredDepts.map(dept => (
            <DepartmentCard 
              key={dept.id} // Use MySQL ID for stable rendering
              dept={dept} 
              isAdmin={isAdmin} 
        ...
        <div className="bg-white rounded-[32px] border-2 border-dashed border-slate-200 p-20 text-center animate-in zoom-in-95 duration-500">
          <Building2 size={48} className="mx-auto text-slate-200 mb-4" />
          <h3 className="text-slate-900 font-black text-lg">No Results Found</h3>
          <p className="text-slate-400 font-medium">No branches found in MySQL. Add a new department to get started.</p>
        </div>
      )}

      {/* 4. SIDE DRAWER (ADD FORM) */}
      {isDrawerOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-md animate-in fade-in" onClick={() => !isSubmitting && setIsDrawerOpen(false)} />
          
          <div className="relative w-full max-w-md bg-white h-full shadow-2xl p-10 animate-in slide-in-from-right duration-500 border-l border-slate-100">
            <div className="flex justify-between items-center mb-10">
              <div className="flex flex-col">
                <h3 className="text-2xl font-black tracking-tight text-slate-900">Create Branch</h3>
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Institutional Unit Setup</span>
              </div>
              <button 
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <Building2 size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" />
                  <input name="deptName" placeholder="e.g. Computer Science" className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3.5 pl-11 pr-4 text-sm font-semibold focus:ring-2 focus:ring-[#136dec]/20 outline-none" required />
                </div>
              </div>

              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Department Code</label>
                <input name="deptCode" placeholder="e.g. CS-2026" className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3.5 px-4 text-sm focus:ring-2 focus:ring-[#136dec]/20 outline-none uppercase font-black text-[#136dec] tracking-widest" required />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <User size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" />
                  <input name="hod" placeholder="Dr. John Doe" className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3.5 pl-11 pr-4 text-sm font-semibold focus:ring-2 focus:ring-[#136dec]/20 outline-none" required />
                </div>
              </div>
              ...
              <div className="pt-6">
                <button 
                  type="submit" 
                  disabled={isSubmitting}
                  className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-xl hover:bg-[#136dec] transition-all hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {isSubmitting ? (
