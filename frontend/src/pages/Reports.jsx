...
// //                 <span className="flex items-center gap-1"><div className="w-2 h-2 bg-cyan-400 rounded-full"></div> Minor</span>
...
//   /* ---------------- DYNAMIC GRAPH LOGIC ---------------- */
//   // This useMemo hook recalculates graph data whenever years change
//   const dynamicGraphData = useMemo(() => {
//     // Logic: Shift data slightly based on the year index to simulate "changing" data
...
//       try {
//         // Note: In a real app, you'd pass the year here: 
//         // axios.get(`.../api/teacher-workload?year=${currentYear}`)
//         const res = await axios.get("http://localhost:5000/api/teacher-workload");

//         const formatted = res.data.map(t => {
//           const major = Number(t.majorCount || 0);
//           const elective = Number(t.electiveCount || 0);
//           const lab = Number(t.labCount || 0);
//           const seminar = Number(t.seminarCount || 0);
          
//           const total = major + elective + lab + seminar;
//           const maxCapacity = 16; 

//           return {
//             name: t.name || "Unknown Teacher",
//             totalSubjects: total,
//             spaceLeft: Math.max(0, maxCapacity - total),
//             percentUsed: (total / maxCapacity) * 100,
//             barColor: total >= 12 ? "bg-red-500" : total >= 8 ? "bg-orange-500" : "bg-blue-500",
//             breakdown: { major, elective, lab, seminar }
//           };
//         });
...
//     fetchWorkload();
//     // Dependency array ensures this re-runs if currentYear changes
//   }, [currentYear]); 
...
//                   className="appearance-none border border-slate-200 rounded-lg px-3 py-1.5 bg-slate-50 w-36 text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-100 transition-all cursor-pointer"
//                 >
...
//         {/* YoY Performance Chart - NOW USING DYNAMIC DATA */}
//         <div className="col-span-12 lg:col-span-8 bg-white p-6 rounded-xl shadow-sm border border-slate-100">
//           <div className="flex justify-between items-center mb-6">
//             <h3 className="font-bold text-lg text-slate-800">Year-over-Year Performance</h3>
//             <div className="flex items-center gap-4 text-[10px] font-bold">
//               <div className="flex items-center gap-1.5">
//                 <span className="w-2 h-2 rounded-full bg-blue-500"></span> {currentYear}
//               </div>
//               <div className="flex items-center gap-1.5">
//                 <span className="w-2 h-2 rounded-full bg-slate-300"></span> {compareYear}
//               </div>
//             </div>
//           </div>
//           <div className="h-64">
...
//         {/* SUBJECTS ASSIGNED */}
//         <div className="col-span-12 lg:col-span-8 bg-white p-6 rounded-xl shadow-sm">
//           <h3 className="font-bold mb-6">Subjects Assigned Per Teacher</h3>
//           <div className="max-h-[260px] overflow-y-auto pr-2 space-y-5 custom-scrollbar">
//             {teacherData.length === 0 ? (
//               <p className="text-sm text-slate-400">Loading teacher data...</p>
//             ) : (
//               teacherData.map((teacher, i) => (
//                 <div key={i} className="group relative">
//                   <div className="flex justify-between text-xs font-bold mb-1">
//                     <span>{teacher.name}</span>
//                     <span>{teacher.totalSubjects}/16 • {teacher.spaceLeft} Slots Left</span>
//                   </div>
//                   <div className="w-full bg-slate-100 h-3 rounded-full overflow-hidden">
//                     <div className={`${teacher.barColor} h-full rounded-full transition-all duration-500`} style={{ width: `${teacher.percentUsed}%` }} />
//                   </div>
//                   <div className="absolute left-0 top-12 hidden group-hover:block bg-white shadow-lg border rounded-lg p-3 text-xs z-50 w-52">
//                     <p className="font-bold mb-2">Subject Breakdown</p>
//                     <p>Major: {teacher.breakdown?.major || 0}</p>
//                     <p>Elective: {teacher.breakdown?.elective || 0}</p>
//                     <p>Lab: {teacher.breakdown?.lab || 0}</p>
//                     <p>Seminar: {teacher.breakdown?.seminar || 0}</p>
//                   </div>
//                 </div>
//               ))
//             )}
...
//         {/* Holistic Metrics */}
//         <div className="col-span-12 bg-white p-6 rounded-xl shadow-sm border border-slate-100">
//            <h3 className="font-bold mb-6">Student Holistic Performance</h3>
//            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
//              {[
//                { label: 'Avg Academic', val: '84%', color: 'bg-blue-500', Icon: GraduationCap },
//                { label: 'NCC Participation', val: '65%', color: 'bg-indigo-500', Icon: Users },
