...
import { 
  Plus, Building2, Users, GraduationCap, BookOpen, ShieldCheck, 
  Bell, Clock, Calendar as CalendarIcon, X, Trash2
} from 'lucide-react';
import StatCard from '../components/StatCard';
...
export default function Overview({ 
  ...
  const [notices, setNotices] = useState([
    { id: 1, day: "24", month: "FEB", year: "2026", title: "Internal Assessment", sub: "All Departments" },
    { id: 2, day: "28", month: "FEB", year: "2026", title: "Staff General Meeting", sub: "Conference Room A" },
    { id: 3, day: "02", month: "MAR", year: "2026", title: "Semester Project Sync", sub: "Labs 1-4" },
  ]);

  const [formData, setFormData] = useState({ title: '', day: '', sub: 'General' });
  ...
  const handleAddNotice = (e) => {
    ...
    const newEntry = {
      ...
      month: monthNames[activeMonthIndex].toUpperCase(),
      year: "2026",
    ...
    setFormData({ title: '', day: '', sub: 'General' });
  };
  ...
  const filteredNotices = notices.filter(n => n.month === monthNames[activeMonthIndex].toUpperCase());

  return (
    <div className="relative animate-in fade-in duration-500 max-w-full font-sans p-2">
      
      {/* 1. HEADER SECTION */}
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
        <div>
          <h1 className="text-[2rem] font-black text-[#0f172a] leading-tight tracking-tight">Dashboard Overview</h1>
          <p className="text-slate-500 text-sm font-medium tracking-tight">
            Welcome back, <span className="text-blue-600 font-bold">{isAdmin ? 'Administrator' : 'Faculty Member'}</span>.
          </p>
        </div>
        <div className="flex gap-3">
          {isAdmin && (
            <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-2 bg-[#136dec] text-white px-5 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-widest shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all active:scale-95">
      ...
      {/* 2. STAT CARDS */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        <StatCard icon={<Building2/>} label="Total Departments" value={departments.length} trend="+Active" color="text-blue-600" />
        <StatCard icon={<Users/>} label="Approved Teachers" value={teachers.length} trend="Live Faculty" color="text-indigo-600" />
        <StatCard icon={<GraduationCap/>} label="Total Students" value={allStudents.length} trend="+Records" color="text-emerald-500" />
        <StatCard icon={<BookOpen/>} label="Total Subjects" value={subjects.length} trend="Curriculum" color="text-amber-500" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-8">
          <div className="bg-white rounded-[24px] border border-slate-200 p-7 shadow-sm">
            <div className="flex justify-between items-center mb-8">
              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Academic Calendar</h4>
              {isAdmin && (
                <button onClick={() => setIsModalOpen(true)} className="p-1.5 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-all">
                  <Plus size={14} />
                </button>
              )}
            </div>
                ...
                {monthNames.map((m, idx) => (
                    <button
                        ...
                        className={`py-2 text-[10px] font-black rounded-lg border transition-all ${
                            ...
                            ? 'bg-[#136dec] border-[#136dec] text-white shadow-md' 
            ...
            <div className="space-y-4">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">Scheduled: {monthNames[activeMonthIndex]} 2026</p>
                {filteredNotices.length > 0 ? (
                    ...
                    <div className="text-center py-10 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                        <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">No notices yet</p>
                    </div>
                )}
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-md rounded-[32px] shadow-2xl p-8 animate-in zoom-in duration-200">
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-black text-slate-900">Add Notice ({monthNames[activeMonthIndex]})</h3>
            ...
            <form onSubmit={handleAddNotice} className="space-y-5">
                <input required type="text" placeholder="Notice Title" className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 font-bold" value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} />
                <div className="grid grid-cols-2 gap-4">
                    <input required type="number" placeholder="Day (DD)" className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 font-bold" value={formData.day} onChange={(e) => setFormData({...formData, day: e.target.value})} />
                    <select className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 font-bold" value={formData.sub} onChange={(e) => setFormData({...formData, sub: e.target.value})}>
                        <option value="General">General</option>
                        <option value="Exams">Exams</option>
                        <option value="Meeting">Meeting</option>
                    </select>
                </div>
                <button type="submit" className="w-full bg-[#136dec] text-white py-4 rounded-xl font-black uppercase tracking-widest hover:bg-blue-700 shadow-lg shadow-blue-200 transition-all">Schedule Notice</button>
            </form>
          </div>
        </div>
      )}
...
const CalendarItem = ({ day, title, sub, active, isAdmin, onDelete }) => (
    <div className={`group flex items-center gap-4 p-4 rounded-2xl border transition-all ${active ? 'bg-[#136dec]/5 border-[#136dec]/10 shadow-sm' : 'border-transparent hover:bg-slate-50'}`}>
      <div className={`size-11 flex items-center justify-center rounded-xl font-black shrink-0 ${active ? 'bg-[#136dec] text-white' : 'bg-slate-100 text-slate-500'}`}>
        {day}
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-[13px] font-black text-slate-800 leading-tight uppercase truncate">{title}</p>
