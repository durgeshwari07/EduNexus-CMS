...
/* Floating Input Component for consistent styling */
...
export default function AdminRegistration() {
  ...
  const handleSubmit = async (e) => {
    ...
    // 2. Map data to match your Backend SQL structure
    const submissionData = {
      collegeName: formData.collegeName,
      collegeCode: formData.collegeCode,
      email: formData.adminEmail, // Primary email for login
      name: formData.adminName,
      username: formData.adminEmail.split('@')[0], // Generates a username from email
      password: formData.password,
      role: "admin"
    };

    try {
      const response = await axios.post("http://localhost:5000/api/register/admin", submissionData);
    ...
    <div className="pt-16 bg-gray-100 min-h-screen">
      <div className="flex items-center justify-center min-h-[calc(100vh-4rem)] p-4">
        <div className="flex flex-col md:flex-row bg-white rounded-3xl shadow-2xl overflow-hidden w-full max-w-6xl md:h-[90vh]">
          ...
          {/* LEFT SECTION - BRANDING */}
          <div className="hidden md:flex flex-1 flex-col justify-between bg-gradient-to-br from-blue-900 to-blue-500 text-white p-12">
            <div className="text-2xl font-extrabold uppercase italic">UniDesk CMS</div>
            <div className="mt-12">
              <h1 className="text-4xl font-extrabold mb-6">Manage Your <br /> Institution with Ease</h1>
              <p className="text-lg opacity-80">Control academics, faculty, and administration from one secure platform.</p>
            </div>
            <div className="text-sm opacity-80 border-t border-blue-400 pt-4">Security-first institutional management system.</div>
          </div>
            ...
            <div className="max-w-2xl mx-auto">
              <div className="mb-8 flex justify-between items-start">
                <div>
                  ...
                  <p className="text-gray-500 mt-2">Register your college or university</p>
                </div>
                <button onClick={() => navigate("/login")} className="text-xs font-bold text-blue-600 uppercase hover:underline">
                  Back to Login
                </button>
              </div>

              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="col-span-2 text-sm font-bold text-blue-500 uppercase">Institutional Details</div>
                
                <div className="col-span-2">
                  <FloatingInput label="College Name" name="collegeName" required onChange={handleChange} />
                </div>
                <FloatingInput label="College Code" name="collegeCode" required onChange={handleChange} />
                <FloatingInput label="College Email" name="collegeEmail" type="email" required onChange={handleChange} />

                <div className="col-span-2">
                  <FloatingInput label="Address / City" name="address" required onChange={handleChange} />
                </div>

                <FloatingInput label="Academic Year" name="academicYear" required onChange={handleChange} />
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full border border-gray-300 rounded-xl px-4 pt-6 pb-2 text-gray-900 bg-white focus:outline-none focus:border-blue-600 appearance-none"
                  >
                    <option value="">Select Board</option>
                    <option>State Board</option>
                    <option>CBSE / ICSE</option>
                    <option>University / Autonomous</option>
                  </select>
                  <label className="absolute left-4 top-2 text-sm text-gray-400">Board</label>
                </div>

                <div className="col-span-2 text-sm font-bold text-blue-500 uppercase border-t pt-4">Admin Info</div>
                <FloatingInput label="Admin Full Name" name="adminName" required onChange={handleChange} />
                <FloatingInput label="Phone Number" name="adminPhone" required onChange={handleChange} />
                
                <div className="col-span-2">
                  <FloatingInput label="Admin Email (Login ID)" name="adminEmail" type="email" required onChange={handleChange} />
                </div>

                <div className="col-span-2 text-sm font-bold text-blue-500 uppercase border-t pt-4">Security</div>
                <FloatingInput label="Password" name="password" type="password" required onChange={handleChange} />
                <FloatingInput label="Confirm Password" name="confirmPassword" type="password" required onChange={handleChange} />

                <button type="submit" className="col-span-2 mt-6 p-4 bg-blue-900 text-white font-extrabold rounded-xl hover:bg-black transition shadow-lg shadow-blue-200">
                  REGISTER INSTITUTION
                </button>
              </form>
            </div>
