...
import React, { useState, useMemo } from 'react';
import { Plus, Edit3, Trash2, Search, Filter, AlertCircle, X, ShieldAlert } from 'lucide-react';
...
export function SubjectsManagement({ data, updateData, readOnly }) {
  ...
  return (
    <div className="animate-in fade-in duration-500 max-w-7xl mx-auto p-8 space-y-8 bg-[#f8fafc] min-h-screen font-sans text-slate-900">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-200 pb-6">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">
            {readOnly ? 'View curriculum and faculty course loads.' : 'Manage curriculum and faculty course loads.'}
          </p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#f1f5f9] border-b border-slate-200">
                ...
                <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-slate-500">Credits</th>
                <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-slate-500">Assigned Faculty</th>
                {/* 🔒 Hide Actions column for teachers */}
                {!readOnly && <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-slate-500 text-center">Actions</th>}
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {filteredSubjects.map((s) => (
      ...
      {/* READ-ONLY INFO FOOTER FOR TEACHERS */}
      {readOnly && (
        <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 flex items-center gap-3">
          <ShieldAlert className="text-blue-600" size={20} />
          <p className="text-xs font-bold text-blue-700 uppercase tracking-tight">
            Read-Only Mode: Only administrators can modify subjects or faculty assignments.
          </p>
        </div>
      )}

      {/* MODALS (Only rendered if not read-only for safety) */}
      {!readOnly && (
        <>
          {/* SUBJECT MODAL */}
          {showModal && (
